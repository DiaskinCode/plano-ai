...
class UserProfile(models.Model):
    ...
    provider = models.CharField(
        max_length=20,
        blank=True,
        help_text="OAuth provider: 'google', 'apple', or None for email/password"
    )
    provider_id = models.CharField(
        ...
        unique=True,
        null=True,
        help_text="Unique ID from OAuth provider (e.g., Google sub, Apple sub)"
    )
    display_name = models.CharField(
        max_length=255,
        blank=True,
        help_text="Display name for the user (from OAuth or custom)"
    )
    avatar_url = models.URLField(
        blank=True,
        help_text="URL to user's profile picture (from OAuth or custom)"
    )

    # Social Profile Fields (Phase 3: LinkedIn-style Social Profiles)
    bio = models.TextField(
        blank=True,
        max_length=500,
        help_text="Short bio about the user (max 500 chars)"
    )
    ...
    target_universities = models.JSONField(
        default=list,
        blank=True,
        help_text='List of target universities: ["Harvard", "MIT", "Stanford"]'
    )

    # Academic Stats (Optional, Privacy-Controlled)
    stats_visibility = models.CharField(
        ...
        default='public',
        help_text="Who can see academic stats (GPA, SAT, etc.)"
    )
    gpa_public = models.DecimalField(
        max_digits=3,
        decimal_places=2,
        null=True,
        blank=True,
        help_text="Public GPA (only shown if stats_visibility allows)"
    )
    sat_score_public = models.IntegerField(
        null=True,
        blank=True,
        help_text="Public SAT score (only shown if stats_visibility allows)"
    )
    ielts_score_public = models.DecimalField(
        max_digits=3,
        decimal_places=1,
        null=True,
        blank=True,
        help_text="Public IELTS score (only shown if stats_visibility allows)"
    )
    ...
    message_privacy = models.CharField(
        ...
        default='everyone',
        help_text="Who can send direct messages"
    )
    activity_visibility = models.CharField(
        ...
        default='public',
        help_text="Who can see user activity (posts, comments, etc.)"
    )

    # Social Stats (Cached counts)
    follower_count = models.IntegerField(
        default=0,
        help_text="Cached follower count"
    )
    following_count = models.IntegerField(
        default=0,
        help_text="Cached following count"
    )
    post_count = models.IntegerField(
        default=0,
        help_text="Cached post count across communities"
    )
    total_likes = models.IntegerField(
        default=0,
        help_text="Cached total upvotes received on posts/comments"
    )
