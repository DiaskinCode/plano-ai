...
def performance_insights(request):
    ...
    Note: Performance analysis feature has been removed as part of
    the university-focused platform transition.
    """
    user = request.user
    profile = user.profile

    # Return basic message since performance analysis is removed
    return Response({
        'message': 'Performance analysis feature has been removed as we focus on university application tracking.',
        'completion_rate': profile.progress_percentage if hasattr(profile, 'progress_percentage') else 0,
    })
