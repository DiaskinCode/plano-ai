...
from . import prompts, prompts_ru
...
class AIService:
    ...
    def __init__(self, provider: str = "openai"):
        ...
        if self.provider == "openai":
            ...
            self.model = (
                "gpt-4o"  # Upgraded from gpt-4o-mini for better personalization
            )
    ...
    def _call_openai(
        ...
        params = {"model": self.model, "messages": messages, "temperature": 0.7}
        ...
        if response_format == "json" and not functions:
            # Only use json_object mode with models that support it
            # gpt-4o and gpt-4o-mini support it, but the API key might have restrictions
            # For safety, only use with specific models
    ...
    def call_llm(
        ...
        if self.provider == "openai":
            ...
            return self._call_anthropic(
                system_prompt, user_prompt, response_format, messages
            )
    ...
    def generate_scenarios(self, user_id: int, user_profile_data: Dict) -> list:
        ...
        cache_key_data = (
            f"{user_profile_data.get('dream_career', '')}"
...
"""
        ...
        user_prompt = p.get_scenario_generation_prompt(
            profile_info + "\n" + context_str
        )
        system_prompt = p.get_system_prompt(
            snapshot["user_meta"].get("coach_character", "supportive")
        )
...
"""
    ...
    def chat_response(
        self, user_id: int, user_message: str, conversation_history: list = None
    ) -> Dict[str, Any]:
...
- DO NOT return arbitrary times - always calculate from current time!"""
            ...
            messages.append(
                {"role": "user", "content": current_message_with_instructions}
            )

            response_text = self.call_llm(
                system_prompt_with_context, messages=messages, response_format="json"
            )
        else:
...
Keep it concise but informative (2-4 sentences)."""
    ...
    def task_specific_assistance(
        ...
        print(
            f"[AI SERVICE] Conversation history length: {len(conversation_history) if conversation_history else 0}"
        )
        print(f"{'=' * 80}\n")
...
Provide detailed, actionable assistance including specific links, contact information, and step-by-step instructions."""
        ...
        response_text = self.call_llm(
            system_prompt, messages=messages, response_format="json"
        )
        ...
        try:
            ...
            result = {
                ...
                "can_expand": mode
                == "clarify",  # Only clarify mode can expand to research
            }
            ...
            if not result["links"] and result["response"]:
                ...
                if extracted_links:
                    print(
                        f"[AI SERVICE] WARNING: AI didn't structure links properly. Extracted {len(extracted_links)} URLs from response text"
                    )
                    result["links"] = extracted_links

            print(
                f"[AI SERVICE] Returning result with {len(result['links'])} links, {len(result['contacts'])} contacts, {len(result['steps'])} steps"
            )
            return result
        except json.JSONDecodeError as e:
...
def get_ai_service():
    """Get or create the AI service singleton instance (lazy initialization)"""
...
class _AIServiceProxy:
    """Proxy for lazy AI service access"""

    def __getattr__(self, name):
        return getattr(get_ai_service(), name)

    def __call__(self, *args, **kwargs):
        return get_ai_service()(*args, **kwargs)
