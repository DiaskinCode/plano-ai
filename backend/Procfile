...
web: bash -c "python manage.py migrate --noinput && echo 'Starting Gunicorn on port ${PORT:-8000}...' && gunicorn pathaibackend.wsgi:application --bind 0.0.0.0:${PORT:-8000} --workers 2 --threads 4 --timeout 660 --log-level debug --capture-output --enable-stdio-inheritance"
